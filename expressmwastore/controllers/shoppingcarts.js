...
exports.addItems = function(req, res){
    ...
    ShoppingCart.findOneAndUpdate({_id : req.params.id}, { $push: {items: product}},{safe: true, upsert: true}, function(err, result){
        if(!err){
            return res.json(result);
        }else{
            return res.send(err);
...
exports.removeItems = function(req, res){
    var product = req.body.product;
    ShoppingCart.findOneAndUpdate({_id : req.params.id}, { $pull: {items: product._id}},{safe: true, upsert: true}, function(err, result){
        if(!err){
            return res.json(result);
        }else{
            return res.send(err);
...
exports.updateQuantity = function(req, res){
    var product = req.body.product;
    ShoppingCart.findOneAndUpdate({_id : req.params.id}, {'items.id': product._id},{'$set': {
        'items.$.quantity': req.body.product.quantity}}, function(err, result){
        if(!err){
            return res.json(result);
        }else{
            return res.send(err);
